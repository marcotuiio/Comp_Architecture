...
.text
	...
	e:
   	...
   	## Fun��o que utiliza a fun��o de indice para percorrer a matriz e assim vai comparando elemento
   	## a elemento e armazenando os valores de interesse
   	## $s1 armazena o menor elemento e $s2 armazena a linha do mesmo
   	## $s3 armazena o maior elemento �mpar e $s4 armazena a linha do mesmo	
   	menor:
   		subi $sp, $sp, 4 # Espa�o para 1 item na pilha
   		sw $ra, ($sp) # Salva o retorno para a main
   		move $a3, $a0 # aux = endere�o base de mat
	...
	max:
		...
		lw $a0, ($v0) # valor em mat[i][j]
   		blt $a0, $s3, next2
   		move $s3, $a0
   		div $s3, $s6  # divis�o do valor atual por 2
